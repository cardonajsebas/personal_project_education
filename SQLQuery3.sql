...
SELECT "Country", "GDP per capita 2020 (USD)" AS 'GDP_per_capita', "Avg Gov exp education % of GDP (last 20 years)" AS 'Avg_gov_exp_edu'
FROM economy
...
WHERE country = 'Jersey'
	...
	-- There was a special case with the Country Ivory Coast, because this contained a semicolon, so I decided to change it on every table:
...
SELECT population."Country", continents."Continent", "GDP per capita 2020 (USD)" AS 'GDP_per_capita'
FROM population LEFT JOIN continents
		ON population.Country LIKE continents.country

SELECT continents.continent, CAST(AVG("GDP per capita 2020 (USD)") AS int) AS 'AVG_GDP_continent'
FROM economy LEFT JOIN continents
		ON economy.Country = continents.country
GROUP BY Continent
ORDER BY AVG_GDP_continent DESC

SELECT continents.subregion, CAST(AVG("GDP per capita 2020 (USD)") AS int) AS 'AVG_GDP_subregion'
FROM economy LEFT JOIN continents
		ON economy.Country = continents.country
GROUP BY subregion
...
SELECT education.Country, continents.continent, continents.subregion, 
...
SELECT education_quality.Country, continents.continent, continents.subregion, 
		education_quality."Avg PISA performance_reading" AS 'reading', 
		education_quality."Avg PISA performance_mathematics" AS 'mathematics', 
		education_quality."Avg PISA performance_science" AS 'science',
		CAST((education_quality."Avg PISA performance_reading" + 
